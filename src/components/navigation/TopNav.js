...
import { useUserSaplings, useSaplingRoutes, useActiveSaplingRoute } from '../../CanopyContext';
...
export const TopNav = () => {
  ...
  const makeSaplingRouteItems = saplingRoutes =>
      saplingRoutes
        .map(({ name, route }) => {
          return (
            <li key={name} className={classnames('sapling-route', {'active': (name === useActiveSaplingRoute().name)})}>
              <a href={route}>{name}</a>
            </li>
          )
        })
    ...
    <>
      <div className="hamburger-menu">
        <ul className="hamburger-items">
          {makeSaplingRouteItems(useSaplingRoutes())}
        </ul>
      </div>
